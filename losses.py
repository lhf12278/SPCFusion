...
class TextureLoss(nn.Module):
    ...
    def forward(self, f_img, v_img):
        ...
        v_i = v_img.repeat([1, 3, 1, 1])
        f_feature = self.loss_network(f_i)
        v_feature = self.loss_network(v_i)
...
class FusionLoss(nn.Module):
    ...
    def forward(self, im_fus, im_ir, im_vi, map_ir, map_vi):
        ...
        return fuse_loss, ms_ssim_loss, l1_loss, grad_loss

def Fusion_loss(ir, vi, fu, mask, weights=[1, 20, 10], device=None):
    ...
    ## 强度损失
    loss_intensity = 0 * final_mse1(ir, vi, fu, mask) + F.l1_loss(fu, torch.max(ir, vi))
...
class Resnet(nn.Module):
    def __init__(self):
        ...
        resnet_raw_model1 = models.resnet18(pretrained=True).cuda()
        self.resnet18_feature_extractor = nn.Sequential(*list(list(resnet_raw_model1.children())[:9]))
...
class P_Fusion_loss(nn.Module):
    def __init__(self, margin=0.3):
        super(P_Fusion_loss, self).__init__()
        self.resnet = Resnet()
        self.triplet_loss = torch.nn.TripletMarginLoss(margin=margin, p=2)

    def forward(self, vi, ir, fu_positive, fu_negative, mask, device=None):
        anchor = ir * mask + vi * (1 - mask)
        anchor = anchor.repeat(1, 3, 1, 1)
        fu_positive = fu_positive.repeat(1, 3, 1, 1)
        fu_negative = fu_negative.repeat(1, 3, 1, 1)
        anchor = self.resnet(anchor)
        positive = self.resnet(fu_positive)
        negative = self.resnet(fu_negative)
...
#         super(P_Fusion_loss, self).__init__()
#         resnet_raw_model1 = models.resnet18(pretrained=True).cuda()
#         self.resnet = resnet_raw_model1.cuda()
#         self.triplet_loss = torch.nn.TripletMarginLoss(margin=margin, p=2)
#
#     def forward(self, vi, ir, fu_positive, fu_negative, mask, device=None):
#         anchor = ir * mask + vi * (1 - mask)
