...
class Saver():
    ...
    def write_img(self, ep, input, output):
        input = torch.cat((input[0][:1, ::].repeat(1, 3, 1, 1), input[1][:1, ::].repeat(1, 3, 1, 1),
                           input[2][:1, ::].repeat(1, 3, 1, 1), input[3][:1, ::].repeat(1, 3, 1, 1)), 3)
        pred = torch.cat((output[0][:1, ::].repeat(1, 3, 1, 1), output[1][:1, ::], output[2][:1, ::],
                          output[3][:1, ::].repeat(1, 3, 1, 1)), 3)
    ...
    def write_model_en(self, ep, total_it, model, optimizer, best_mIoU, device, is_best=True, logger=None):
        model = model.cpu()
        state = {'MTAN': model.state_dict(),
                 'optimizer': optimizer.state_dict(),
                 'ep': ep,
        ...
        # save_path = os.path.join(self.model_dir, '{:0>5d}.pth'.format(ep))
        if is_best:
            save_path = os.path.join(self.model_dir, 'encoder_best.pth'.format(ep))
        else:
            save_path = os.path.join(self.model_dir, '{:0>5d}_encoder.pth'.format(ep))
        torch.save(state, save_path)
        # logger.info('Save model to: {}'.format(save_path))
        model.to(device)

    def write_model_de(self, ep, total_it, model, optimizer, best_mIoU, device, is_best=True, logger=None):
        model = model.cpu()
        state = {'MTAN': model.state_dict(),
                 'optimizer': optimizer.state_dict(),
                 'ep': ep,
                 ...
                 'best_mIou': best_mIoU}
        if is_best:
            save_path = os.path.join(self.model_dir, 'decoder_best.pth'.format(ep))
        else:
            save_path = os.path.join(self.model_dir, '{:0>5d}_decoder.pth'.format(ep))
        torch.save(state, save_path)
        # logger.info('Save model to: {}'.format(save_path))
        model.to(device)

        def write_model_en_p(self, ep, total_it, model, optimizer, best_mIoU, device, is_best=True, logger=None):
            model = model.cpu()
            state = {'MTAN': model.state_dict(),
                     'optimizer': optimizer.state_dict(),
                     'ep': ep,
            ...
            # save_path = os.path.join(self.model_dir, '{:0>5d}.pth'.format(ep))
            if is_best:
                save_path = os.path.join(self.model_dir, 'encoder_pro_best.pth'.format(ep))
            else:
                save_path = os.path.join(self.model_dir, '{:0>5d}_decoder_pro.pth'.format(ep))
            torch.save(state, save_path)
            # logger.info('Save model to: {}'.format(save_path))
            model.to(device)
