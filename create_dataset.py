...
class MSRSData(torch.utils.data.Dataset):
    ...
    def __init__(self, opts, is_train=True, crop=lambda x: x):
        ...
        if is_train:
            ...
            self.vismask_folder = os.path.join(opts.dataroot, 'train', 'vi_map')
            self.irmask_folder = os.path.join(opts.dataroot, 'train', 'ir_map')
    ...
    def __getitem__(self, index):
        ...
        irmask_path = os.path.join(self.vismask_folder, image_name)
        vimask_path = os.path.join(self.irmask_folder, image_name)
        ...
        vis_mask = self.imread(path=vimask_path, vis_flage=False)
        ir_mask = self.imread(path=irmask_path, vis_flage=False)
        ...
        if self.is_train:
            ...
            vis_ir = torch.cat([vis, ir, vis_mask, ir_mask, label, bi, bd, mask], dim=1)
            ...
            vis, ir, vis_mask, ir_mask, label, bi, bd, mask = torch.split(patch, [3, 1, 1, 1, 1, 1, 1, 1], dim=1)
            ...
            ## ir 单通道, vis RGB三通道
            return ir.squeeze(0), vis.squeeze(0), ir_mask.squeeze(0), vis_mask.squeeze(), label.squeeze(0), bi.squeeze(0), bd.squeeze(0), mask.squeeze(0)
        else: 
            label = label.type(torch.LongTensor)
            return ir.squeeze(0), vis.squeeze(0), ir_mask.squeeze(0), vis_mask.squeeze(), label.squeeze(0), image_name
